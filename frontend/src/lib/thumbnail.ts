// Board-list thumbnail renderer (step 12). Ports the SVG string-building
// logic from mcp-server/src/render/svg.ts, but reuses frontend's own
// lib/geometry.ts and lib/types.ts directly instead of duplicating them
...
function renderSticky(shape: Extract<Shape, { type: 'sticky' }>): string {
  ...
  if (shape.text) {
    ...
    for (const line of lines) {
      if (ty > shape.y + shape.height - 2) break;
      parts.push(
...
function renderArrow(from: { x: number; y: number }, to: { x: number; y: number }, pending: boolean): string {
...
function shapeAABB(shape: Shape, getShape: (id: string) => Shape | undefined): Bounds | null {
  if (shape.type === 'arrow') {
...
function computeViewBox(shapes: Shape[], getShape: (id: string) => Shape | undefined): Bounds {
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
  let maxY = -Infinity;
  for (const shape of shapes) {
    const b = shapeAABB(shape, getShape);
    if (!b) continue;
  ...
  if (!Number.isFinite(minX)) return { x: 0, y: 0, width: 480, height: 320 };
  return {
    x: minX - PADDING,
    y: minY - PADDING,
...
// data: URI for direct use as an <img src> — cheaper than an offscreen
// canvas render for a small list thumbnail, and the SVG is already
// XML-escaped so it's safe to inline.
export function shapeGraphToThumbnailUri(graph: Record<string, Shape>): string {
